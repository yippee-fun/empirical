...
class Empirical::SignatureProcessor < Empirical::BaseProcessor
	...
	def visit_call_node(node)
		case node
		in Prism::CallNode[name: :fun, receiver: nil, arguments: Prism::ArgumentsNode[
			arguments: [
				Prism::KeywordHashNode[
					elements: [
						Prism::AssocNode[key: Prism::CallNode | Prism::LocalVariableReadNode | Prism::ConstantReadNode]
					]
				]
			]
		]]
	...
	def visit_fun_call_node(node)
		...
		raise SyntaxError unless node in {
		...
		compile_function(
			...
			signature,
			return_type,
			node.block || @block_stack.first.block
		)
	end

	def compile_function(node, signature, return_type, body_block)
		...
		defaults = body_block.parameters&.parameters&.keywords.to_h do |param|
			...
			when Prism::OptionalKeywordParameterNode
				...
				raise "Incorrect default parameters."
			end
		end
		...
		in Prism::CallNode
			...
			signature.arguments&.arguments&.each do |argument|
				...
				in Prism::LocalVariableWriteNode[name: name, value: typed_param]
					...
					default_string = defaults[name.name]&.slice
					...
					if default_string
						...
						@annotations << [
							(start = argument.name_loc.end_offset),
							typed_param.location.end_offset - start,
							"",
						]
					end
					...
					argument.elements.each do |argument|
						...
						in Prism::HashNode[elements: [Prism::AssocNode[key: key_type, value: value_type]]]
							...
							if default_string
								...
								@annotations << [
									(start = argument.key.closing_loc.start_offset),
									typed_param.location.end_offset - start,
									"",
								]
							end
